...
export function TransactionDrawer({ onClose }: { onClose: () => void }) {
  ...
  function TransactionItem({ tx }: { tx: import('./types').TrackedTransaction }) {
    const displayPhase = tx.phase === 'TIMEOUT' ? 'TRACKING DELAYED' : tx.phase;
    const displayMessage = displayPhase === 'TRACKING DELAYED'
      ? 'Transaction status is temporarily unavailable. PALINODE will continue tracking this transaction by ID.'
      : displayPhase === 'FINALIZED ERROR'
        ? 'Transaction finalized with an execution error.'
        : null;
    return <div className="tx-item"><div className="tx-item-head"><strong>{tx.label}</strong><StatusPill value={displayPhase} /></div><div className="tx-method">{tx.method}</div><div className="tx-id mono">{truncate(tx.id, 14, 10)} <CopyButton value={tx.id} compact /></div><div className="tx-progress"><span className={tx.phase.includes('FINALIZED SUCCESS') ? 'done' : tx.phase.includes('ERROR') || tx.phase === 'UNDETERMINED' || tx.phase === 'CANCELED' ? 'bad' : 'current'} /></div><div className="tx-meta"><span>STORED {tx.protocolStatus}</span><span>{tx.executionResult}</span><span>{formatDate(tx.updatedAt)}</span></div>{tx.resolutionAction === 'Finalize' && <div className="tx-action">FINALIZATION AVAILABLE · action=Finalize</div>}{displayMessage && <div className="tx-error">{displayMessage}</div>}</div>;
  }
