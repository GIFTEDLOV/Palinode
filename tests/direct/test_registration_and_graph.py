...
def evidence(contract, uri=EVIDENCE_URI, body=b"registered evidence", subject="case-1", title="Evidence"):
    return contract.register_evidence(uri, digest(body), len(body), subject, title)


def test_registers_all_node_types_and_exposes_immutable_metadata(direct_deploy):
...
def test_duplicate_and_malformed_registration_reverts(direct_vm, direct_deploy):
    ...
    with direct_vm.expect_revert("duplicate evidence identity"):
        evidence(contract)
    with direct_vm.expect_revert("invalid HTTPS source URI"):
...
def test_lineage_preserves_old_evidence_and_marks_successor(direct_vm, direct_deploy):
    ...
    old_id = evidence(contract, body=old_body, title="Old")
    new_id = evidence(
        contract,
